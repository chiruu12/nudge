...
description: |
  ...
  Use when setting up a new project for AI-assisted development, when Codex keeps
  ...
  "how to set up Codex for this project".
---
...
- Codex keeps forgetting conventions or making the same mistakes
...
ls -la AGENTS.md .Codex/settings.json .Codex/rules/ 2>/dev/null
...
|-------|------|-------------|----------|
| **1. Rules files** | Non-negotiable constraints | Always (every request) | AGENTS.md, .Codex/rules/*.md |
...
AGENTS.md and .Codex/rules/ are always in context. They should contain:
...
- Key decisions already made (so Codex doesn't re-litigate)
- Acceptance criteria
- Out of scope

Link from AGENTS.md: "For feature X, see `docs/specs/feature-x.md`"

### 5. Optimize for token efficiency

- Move detailed reference material to files Codex reads on-demand (Level 3)
- Keep Level 1 rules under 2000 words total
- Use progressive disclosure — summary in AGENTS.md, details in linked files
- Remove stale context — outdated rules cause confusion, not just waste

### 6. Verify context effectiveness

Test that the context is working by asking Codex to perform a common task without any additional explanation. If it follows conventions correctly, the context is sufficient.
...
| "I'll put everything in AGENTS.md so it's always available" | AGENTS.md is loaded on EVERY request. A 5000-word AGENTS.md burns tokens even for simple questions. Use progressive disclosure — summary at Level 1, details at Level 3. |
| "The code is self-documenting, no rules needed" | Code shows WHAT exists, not WHY it's that way or WHAT to do next time. Without explicit rules, Codex will infer conventions from whatever code it reads first — which may be the exception, not the rule. |
...
- AGENTS.md over 3000 words — too much always-loaded context, burning tokens on every request
- Rules contradict each other — Codex will follow whichever it sees last, which is non-deterministic
- Stale rules reference deleted files or patterns — causes confusion and hallucination
- No Level 2 specs for active features — Codex fills gaps with assumptions

## Verification Checklist

- [ ] AGENTS.md under 2000 words and contains only non-negotiable constraints
- [ ] .Codex/rules/ files cover common workflows without being overly prescriptive
- [ ] Active features have Level 2 spec files linked from AGENTS.md
- [ ] Codex follows project conventions without re-explanation on a fresh session
- [ ] No contradictions between rules files
- [ ] Stale context removed (no references to deleted files/patterns)

## Anti-patterns

- **DO NOT** put everything in AGENTS.md — use progressive disclosure across all 5 levels
